...
function App(){
 ...
 <Section id="details" eyebrow="The Celebration" title="Save our special date"><Countdown/><div className="timeline">{timeline.map(([time,name,desc],i)=><div className="event" key={name}><span>{time}</span><b>{name}</b><p>{desc}</p>{i < timeline.length - 1 && <i />}</div>)}</div></Section>
 <Section id="venue" eyebrow="The Venue" title="Where our forever begins"><div className="venue-card glass"><div><MapPin size={25}/><h3>VP House</h3><p>Mattool South<br/>Kannur, 670302</p><a className="text-link" target="_blank" rel="noreferrer" href="https://www.google.com/maps/place/Matool,+Kerala+670302,+India/@32.7040107,22.0917581,5205615m/data=!3m1!1e3!4m10!1m3!11m2!2s_JIEFHhslCr0Naj36qpP0uUPVe8_RA!3e1!3m5!1s0x3ba4143228e42271:0xe4067110a53d77b!8m2!3d11.966825!4d75.2889789!16s%2Fg%2F11t7r2dx65?entry=ttu&g_ep=EgoyMDI2MDcxNS4wIKXMDSoASAFQAw%3D%3D">Get directions ↗</a></div><div className="map"><iframe title="VP House Map" src="https://www.google.com/maps?q=Mattool%20South%20Kannur%20Kerala%20670302&output=embed" loading="lazy" /></div></div></Section>
 ...
 <Section id="rsvp" eyebrow="Will You Join Us?" title="Your presence is our present"><p className="lead">Your presence is the greatest gift to us. We look forward to celebrating this joyous occasion together.</p>{submitted?<div className="thank-you glass"><Heart fill="currentColor"/><h3>Thank you for your response</h3><p>We’re so happy you’ll be part of our day.</p></div>:<form className="rsvp glass" onSubmit={e=>{e.preventDefault();setSubmitted(true)}}><label>Full name<input required placeholder="Your name"/></label><label>Will you attend?<select required defaultValue=""><option value="" disabled>Select your response</option><option>Joyfully accepts</option><option>Regretfully declines</option></select></label><label>Number of guests<select defaultValue="1"><option>1</option><option>2</option><option>3</option><option>4</option></select></label><label>Message <textarea placeholder="Leave a note for the couple" rows="3"/></label><button className="button" type="submit">Send RSVP <Send size={16}/></button></form>}</Section>
